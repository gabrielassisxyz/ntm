...
func TestIsolateUserConfigProcessPreservesConfiguredEnvironment(t *testing.T) {
	...
	// Must match tmuxenv.Pattern: NTM_TEST_TMUX_ENV_OWNED is only trusted
	// when TMUX_TMPDIR proves it, not merely because it is set.
	t.Setenv("TMUX_TMPDIR", ShortTmuxTempDir(t))
	ownedHome := t.TempDir()
...
// TestIsolateUserConfigProcessIgnoresUnprovenOwnershipFlag is the regression
// case for the same reason TestIsolateTmuxTestProcessIgnoresUnprovenOwnershipFlag
// exists: an ambient NTM_TEST_TMUX_ENV_OWNED=1 left over from an earlier
// command must not by itself disable HOME/XDG_CONFIG_HOME isolation. Before
// requiring TmuxTestEnvOwned this combination short-circuited to a no-op and
// pointed the whole suite at the developer's real ~/.config.
func TestIsolateUserConfigProcessIgnoresUnprovenOwnershipFlag(t *testing.T) {
	realHome, err := os.UserHomeDir()
	if err != nil {
		t.Fatalf("UserHomeDir: %v", err)
	}
	t.Setenv("NTM_TEST_TMUX_ENV_OWNED", "1")
	t.Setenv("TMUX_TMPDIR", "") // unset: does not match tmuxenv.Pattern
	...
	if got := os.Getenv("HOME"); got == "" || got == realHome {
		t.Fatalf("HOME = %q, want isolation to run despite the unproven flag", got)
	}
	if got := os.Getenv("XDG_CONFIG_HOME"); got == "" || strings.HasPrefix(got, realHome) {
		t.Fatalf("XDG_CONFIG_HOME = %q, want isolation to run despite the unproven flag", got)
	}

	// A TMUX_TMPDIR that is merely set, but to something tmuxenv.Pattern does
	// not match, must be treated the same as unset.
	if err := cleanup(); err != nil {
	...
	if got := os.Getenv("HOME"); got == "" || got == realHome {
		t.Fatalf("HOME = %q, want isolation to run despite the unmatched TMUX_TMPDIR", got)
	}
}
